...
class SpatialMeanNUMPY():
    """ 
    Keras Layer: Spatial Mean (mean coordinate of non-neg. weight tensor)
    IGNORES CHANNELS CURRENTLY (so...tf.map_fn?)
    This computes offset from CENTER of 0th index voxel.
    """

    def __init__(self, input_shape, eps=1e-9):
        coord_idxs = []
        input_shape_nb_nc = input_shape
        self.eps = eps

        for idx, in_shape in enumerate(input_shape_nb_nc):
            coord_idx_tensor = np.arange(in_shape)
            coord_idx_tensor = np.reshape(
                coord_idx_tensor, [in_shape] + [1]*(len(input_shape_nb_nc)-1))

            coord_idx_tensor = np.tile(
                coord_idx_tensor,
                reps=[1] + input_shape_nb_nc[:idx] + input_shape_nb_nc[idx+1:]
            )

            coord_idx_tensor = np.transpose(
                coord_idx_tensor,
                list(range(1, idx+1)) +
                [0] + list(range(idx+1, len(input_shape_nb_nc)))
            )

            coord_idxs.append(
                np.reshape(coord_idx_tensor.astype(np.float32), [-1])
            )
        self.coord_idxs = np.stack(coord_idxs)

    def __call__(self, x):
        x = np.reshape(x, [-1])
        # print(self.coord_idxs)
        return np.sum(self.coord_idxs * x, axis=1) / np.sum(x + self.eps)
